$.AmsRecord = {
    ...
    authROOT:[],
    authRow : undefined,
    AmsBusinessAuth:undefined,
    init:function(){
        var autoQuery = false;
        if ($("#amsRecord_master").length>0){
            if ($("#amsRecord_master").panel("options").region=="north"){
                $("#amsRecord_master").panel("resize",{height:document.body.clientHeight});
            }else{
                $("#amsRecord_master").panel("resize",{width:document.body.clientWidth});
            }
        }
        ...
        if ($('#AmsBusinessMemberGridPage').length>0){
            $.totemUtils.setHeight($('#AmsBusinessMemberGridPage'));
        }

        if ($('#AmsBusinessMemberGrid').length>0){
            $('#AmsBusinessMemberGrid').datagrid({
    ...
    updateTab:function (tab) {
        ...
        if (null!=content&&content.indexOf("iframe")<0){
            ...
            $('#AmsRecordMemberTabs').tabs('update',{
                tab:tab,
                options:{
                    content:content
                }
...
$(function() {
    $("#AmsRecordMemberTabs").panel({
        onResize:function(width,height){
            if ($("#amsRecord_master").panel("options").region!="north"){
                return;
            }
            height = height - 30;
            if ($("#AmsBusinessFrame").length>0){
                tab = $('#AmsRecordMemberTabs').tabs('getTab',"档案业务");
                $("#AmsBusinessFrame").css("height",height);
                tab.css("height",height+4);
                $.totemUtils.setHeight(document.getElementById("AmsBusinessFrame").contentWindow.$("#AmsBusinessGridPage"));
            }
        }
    })
    if ($('#AmsRecordGrid').length>0){
        $('#AmsRecordGrid').datagrid({
            ...
            onSelect:function(index,row){
                ...
                if ($('#AmsRecordMemberTabs').length>0){
                    if ($('#AmsRecordMemberTabs').tabs("tabs").length>1){
                        $('#AmsRecordMemberTabs').tabs("showHeader");
                    }
                }
                var height = $('#AmsRecordMemberTabs').outerHeight();
                if ($('#AmsRecordMemberTabs').tabs('exists',"档案业务")){
                    var tab = $('#AmsRecordMemberTabs').tabs('getTab',"档案业务");
                    tab.css("height",height+3);
                    var para = "?userNo="+row.userNo;
    ...
    if ($("#AmsRecordMemberTabs").length>0){
        $("#AmsRecordMemberTabs").tabs({
            onSelect:function (title,index) {
                $.AmsRecord.updateTab($('#AmsRecordMemberTabs').tabs('getTab',index));
            },
            onUpdate:function (title,index) {
                var tab = $('#AmsRecordMemberTabs').tabs('getTab',index);
                if (tab.is(":visible")){
                    $.AmsRecord.updateTab(tab);
                }
            }
        })
    }
    $('#AmsBusinessWindow').window({
        closed:true,
        onClose:function () {
            if (document.getElementById("AmsBusinessFrame").contentWindow.$("#AmsBusinessGrid").length>0){
                document.getElementById("AmsBusinessFrame").contentWindow.$.AmsBusiness.search(document.getElementById("AmsBusinessFrame").contentWindow.$("#AmsBusinessGrid"))
            }else{
                document.getElementById("AmsBusinessFrame").contentWindow.$.AmsBusiness.search(document.getElementById("AmsBusinessFrame").contentWindow.$("#AmsBusinessTreeGrid"))
            }
        }
    })
