$.AmsOtherrecordin = {
    ...
    init:function(){
        var autoQuery = false;
        if ($("#amsOtherrecordin_master").length>0){
            if ($("#amsOtherrecordin_master").panel("options").region=="north"){
                $("#amsOtherrecordin_master").panel("resize",{height:document.body.clientHeight});
            }else{
                $("#amsOtherrecordin_master").panel("resize",{width:document.body.clientWidth});
            }
        }
    ...
    updateTab:function (tab) {
        ...
        if (null!=content&&content.indexOf("iframe")<0){
            ...
            $('#AmsOtherrecordinMemberTabs').tabs('update',{
                tab:tab,
                options:{
                    content:content
                }
...
$(function() {
    $("#AmsOtherrecordinMemberTabs").panel({
    ...
    if ($('#AmsOtherrecordinGrid').length>0){
        $('#AmsOtherrecordinGrid').datagrid({
            ...
            onSelect:function(index,row){
                ...
                if ($('#AmsOtherrecordinMemberTabs').length>0){
                    if ($('#AmsOtherrecordinMemberTabs').tabs("tabs").length>1){
                        $('#AmsOtherrecordinMemberTabs').tabs("showHeader");
                    }
                }
                var height = $('#AmsOtherrecordinMemberTabs').outerHeight() - 30;
    ...
    if ($("#AmsOtherrecordinMemberTabs").length>0){
        $("#AmsOtherrecordinMemberTabs").tabs({
            onSelect:function (title,index) {
                $.AmsOtherrecordin.updateTab($('#AmsOtherrecordinMemberTabs').tabs('getTab',index));
            },
            onUpdate:function (title,index) {
                var tab = $('#AmsOtherrecordinMemberTabs').tabs('getTab',index);
                if (tab.is(":visible")){
                    $.AmsOtherrecordin.updateTab(tab);
                }
            }
        })
    }
