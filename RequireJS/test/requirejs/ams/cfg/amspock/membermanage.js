$.AmsPock = {
    ...
    authROOT:[],
    authRow : undefined,
    AmsLowrecordinAuth:undefined,
    init:function(){
        var autoQuery = false;
        if ($("#amsPock_master").length>0){
            if ($("#amsPock_master").panel("options").region=="north"){
                $("#amsPock_master").panel("resize",{height:document.body.clientHeight});
            }else{
                $("#amsPock_master").panel("resize",{width:document.body.clientWidth});
            }
        }
        ...
        if ($('#AmsLowrecordinMemberGridPage').length>0){
            $.totemUtils.setHeight($('#AmsLowrecordinMemberGridPage'));
        }

        if ($('#AmsLowrecordinMemberGrid').length>0){
            $('#AmsLowrecordinMemberGrid').datagrid({
...
$(function() {
    $("#AmsPockMemberTabs").panel({
        onResize:function(width,height){
            if ($("#amsPock_master").panel("options").region!="north"){
                return;
            }
            height = height - 30;
            if ($("#AmsLowrecordinFrame").length>0){
                tab = $('#AmsPockMemberTabs').tabs('getTab',"低压档案入库");
                $("#AmsLowrecordinFrame").css("height",height);
                tab.css("height",height+4);
                $.totemUtils.setHeight(document.getElementById("AmsLowrecordinFrame").contentWindow.$("#AmsLowrecordinGridPage"));
            }
        }
    })
    if ($('#AmsPockGrid').length>0){
        $('#AmsPockGrid').datagrid({
            ...
            onSelect:function(index,row){
                ...
                if ($('#AmsPockMemberTabs').length>0){
                    if ($('#AmsPockMemberTabs').tabs("tabs").length>1){
                        $('#AmsPockMemberTabs').tabs("showHeader");
                    }
                }
                var height = $('#AmsPockMemberTabs').outerHeight();
                if ($('#AmsPockMemberTabs').tabs('exists',"低压档案入库")){
                    var tab = $('#AmsPockMemberTabs').tabs('getTab',"低压档案入库");
    ...
    if ($("#AmsPockMemberTabs").length>0){
        $("#AmsPockMemberTabs").tabs({
            onSelect:function (title,index) {
                $.AmsPock.updateTab($('#AmsPockMemberTabs').tabs('getTab',index));
            },
            onUpdate:function (title,index) {
                var tab = $('#AmsPockMemberTabs').tabs('getTab',index);
                if (tab.is(":visible")){
                    $.AmsPock.updateTab(tab);
                }
            }
        })
    }
