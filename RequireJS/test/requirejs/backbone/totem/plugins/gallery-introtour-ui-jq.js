...
(function($) {
	...
	var privateVars = {}, introTourClosed = new CustomEvent("introTourClosed"), introTourLastCard = new CustomEvent(
			'introTourLastCard'), 
	findpos = function(obj) {
		...
		if (obj.offsetParent) {
			...
			curleft += obj.parentNode.offsetLeft;
			curtop += obj.parentNode.offsetTop+obj.parentNode.offsetHeight;
		}
	...
	ATTRS = {
		...
		'buttonwelcome' : {
			'content' : '开始',
			'buttonid' : 'yui-galleryintrotourui-buttonwelcome-id'
		},
		'buttontourend' : {
			'content' : '关闭',
			'buttonid' : 'yui-galleryintrotourui-buttontourend-id'
		},
		'buttonnav' : {
			'content' : '下一个',
			'buttonid' : 'yui-galleryintrotourui-buttonnav-'
		}
		...
		if (!cardstyle) {
			...
			if (!cardstyle.button) {
				cardstyle.button = defAttrs.button;
			}
			if (!cardstyle.buttontext) {
				cardstyle.buttontext = defAttrs.buttontext;
			}
			if (!cardstyle.title) {
				cardstyle.title = defAttrs.title;
			}
			if (!cardstyle.content) {
				cardstyle.content = defAttrs.content;
			}
			if (!cardstyle.cardborder) {
				cardstyle.border = defAttrs.border;
			}
		}
		return cardstyle;
	}, slideTemplate = function(ci, button, seqid, index) {
		var buttonid = button.buttonid, html = "", arrowclass = "";
		if (!ci.title) {
			ci.title = "";
		}
		if (!ci.content) {
		...
		if (ci.position === "right") {
			arrowclass = "right";
		} else if (ci.position === "left") {
			arrowclass = "left";
		} else if (ci.position === "top") {
			arrowclass = "top";
		} else if (ci.position === "bottom") {
			arrowclass = "bottom";
		}
		if (arrowclass) {
			html = "<div class='yui-galleryintrotourui-card-arrow "
					+ arrowclass + "'></div>";
		}
		html += "<div class='yui-galleryintrotourui-card-container'>"
				...
				+ "<div class='yui-galleyintroui-card-title'>"
				+ ci.title
				+ "</div>"
				+ "<div class='yui-galleryintrotourui-card-content'>"
				+ ci.content
				+ "</div>"
				+ "</div>"
				+ "<div class='yui-galleryintrotourui-card-nav'>"
				+ "<input type='checkbox' name='close-nav'>不再显示</input>"
				+ "<button data-seqid='"
				+ seqid
				+ "' id='"
				+ buttonid
				+ "' class='yui-galleryintrotourui-card-next yui3-button notice'>"
				+ button.content + "</button>" + "</div>" + "</div>";

		return html;
	}, generateSlideDom = function(toppos, leftpos, ci, id, type, seqid, index) {
		var button = {}, html;
		if (!type) {
			button = ATTRS.buttonnav;
		} else if (type === "welcome") {
			button = ATTRS.buttonwelcome;
			seqid = "welcome";
		} else if (type === "end") {
			button = ATTRS.buttontourend;
			seqid = "end";
		}
		html = slideTemplate(ci, button, seqid, index), node = $('<div></div>');
		...
		if ((ci.position === 'left' || ci.position === 'right')
				&& ci.width === 'undefined') {
			ci.width = 0;
		}
		if ((ci.position === 'top' || ci.position === 'bottom')
				&& ci.height === 'undefined') {
			ci.width = 0;
		}
	...
	$.fn.introTour = function(cardinfo, cardstyle) {
		$.each($("div.yui-galleryintrotourui-card"),function(index,obj){
			obj.outerHTML = "";
		});
		privateVars.hscroll = (document.all ? document.scrollLeft
				: window.pageXOffset),
				privateVars.vscroll = (document.all ? document.scrollTop
						: window.pageYOffset);
		privateVars.prevActiveElement = document.activeElement;
		cardstyle = setcardstyle(cardstyle);
		generateSlideDom("60px", "50%", cardinfo[0],
				'galleryintrotourui-card-welcome', 'welcome', 0);
		for (var i = 1; i < cardinfo.length; i++) {
			var ci = cardinfo[i], elem = document.getElementById(ci.target), pos, index = i;
			if (elem) {
				pos = findpos(elem), id = 'galleryintrotourui-card-' + i;
				pos = getCardPos(ci, pos);
				pos[1] = pos[1] + "px";
				pos[0] = pos[0] + "px";
				if (i === cardinfo.length - 2) {
					i = "end";
				}
				generateSlideDom(pos[1], pos[0], ci, id, '', i, index);
			}
		}
		generateSlideDom("60px", "50%", cardinfo[cardinfo.length - 1],
				'galleryintrotourui-card-endtour', 'end', 0);
		window.scrollTo(0, 0);
		...
		$(".yui-galleryintrotourui-card-next")
				.click(
						function() {
							var seqid = this.getAttribute("data-seqid"), carddivid = "", buttondivid = "";
							$(this).blur();
							$(".yui-galleryintrotourui-card").css("display",
									"none");
							if (seqid === "welcome") {
								...
								buttondivid = '#yui-galleryintrotourui-buttonnav-'
										+ seqid;
							}
							if (this.getAttribute("id") === "yui-galleryintrotourui-buttontourend-id") {
								...
								$(buttondivid).focus();
								if (seqid !== "end") {
									var toppos = $(carddivid).css('top'), leftpos = $(
											carddivid).css('left');
