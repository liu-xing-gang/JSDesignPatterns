$.AmsMovableRack = {
    ...
    authROOT:[],
    authRow : undefined,
    AmsRackNoAuth:undefined,
    init:function(){
        var autoQuery = false;
        if ($("#amsMovableRack_master").length>0){
            if ($("#amsMovableRack_master").panel("options").region=="north"){
                $("#amsMovableRack_master").panel("resize",{height:document.body.clientHeight});
            }else{
                $("#amsMovableRack_master").panel("resize",{width:document.body.clientWidth});
            }
        }
        ...
        if ($('#AmsRackNoMemberGridPage').length>0){
            $.totemUtils.setHeight($('#AmsRackNoMemberGridPage'));
        }

        if ($('#AmsRackNoMemberGrid').length>0){
            $('#AmsRackNoMemberGrid').datagrid({
    ...
    updateTab:function (tab) {
        ...
        if (null!=content&&content.indexOf("iframe")<0){
            ...
            $('#AmsMovableRackMemberTabs').tabs('update',{
                tab:tab,
                options:{
                    content:content
                }
...
$(function() {
    $("#AmsMovableRackMemberTabs").panel({
        onResize:function(width,height){
            if ($("#amsMovableRack_master").panel("options").region!="north"){
                return;
            }
            height = height - 30;
            if ($("#AmsRackNoFrame").length>0){
                tab = $('#AmsMovableRackMemberTabs').tabs('getTab',"档案位管理");
                $("#AmsRackNoFrame").css("height",height);
                tab.css("height",height+4);
                $.totemUtils.setHeight(document.getElementById("AmsRackNoFrame").contentWindow.$("#AmsRackNoGridPage"));
            }
        }
    })
    if ($('#AmsMovableRackGrid').length>0){
        $('#AmsMovableRackGrid').datagrid({
            ...
            onSelect:function(index,row){
                ...
                if ($('#AmsMovableRackMemberTabs').length>0){
                    if ($('#AmsMovableRackMemberTabs').tabs("tabs").length>1){
                        $('#AmsMovableRackMemberTabs').tabs("showHeader");
                    }
                }
                var height = $('#AmsMovableRackMemberTabs').outerHeight();
                if ($('#AmsMovableRackMemberTabs').tabs('exists',"档案位管理")){
                    var tab = $('#AmsMovableRackMemberTabs').tabs('getTab',"档案位管理");
                    tab.css("height",height+3);
                    var para = "?movableRack="+row.movableRackId;
    ...
    if ($("#AmsMovableRackMemberTabs").length>0){
        $("#AmsMovableRackMemberTabs").tabs({
            onSelect:function (title,index) {
                $.AmsMovableRack.updateTab($('#AmsMovableRackMemberTabs').tabs('getTab',index));
            },
            onUpdate:function (title,index) {
                var tab = $('#AmsMovableRackMemberTabs').tabs('getTab',index);
                if (tab.is(":visible")){
                    $.AmsMovableRack.updateTab(tab);
                }
            }
        })
    }
