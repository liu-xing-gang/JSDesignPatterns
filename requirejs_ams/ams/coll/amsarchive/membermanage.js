$.AmsArchive = {
    ...
    authROOT:undefined,
    authRow : undefined,
    AmsFileAuth:undefined,
    init:function(){
        var autoQuery = false;
        if ($("#amsArchive_master").length>0){
            if ($("#amsArchive_master").panel("options").region=="north"){
                $("#amsArchive_master").panel("resize",{height:document.body.clientHeight});
            }else{
                $("#amsArchive_master").panel("resize",{width:document.body.clientWidth});
            }
        }
        ...
        if ($('#AmsFileMemberGridPage').length>0){
            $.totemUtils.setHeight($('#AmsFileMemberGridPage'));
        }

        if ($('#AmsFileMemberGrid').length>0){
            $('#AmsFileMemberGrid').datagrid({
    ...
    updateTab:function (tab) {
        ...
        if (null!=content&&content.indexOf("iframe")<0){
            ...
            $('#AmsArchiveMemberTabs').tabs('update',{
                tab:tab,
                options:{
                    content:content
                }
...
$(function() {
    $("#AmsArchiveMemberTabs").panel({
        onResize:function(width,height){
            if ($("#amsArchive_master").panel("options").region!="north"){
                return;
            }
            height = height - 30;
            if ($("#AmsFileFrame").length>0){
                tab = $('#AmsArchiveMemberTabs').tabs('getTab',"原文管理");
                $("#AmsFileFrame").css("height",height);
                tab.css("height",height+4);
                $.totemUtils.setHeight(document.getElementById("AmsFileFrame").contentWindow.$("#AmsFileGridPage"));
            }
        }
    })
    if ($('#AmsArchiveGrid').length>0){
        $('#AmsArchiveGrid').datagrid({
            ...
            onSelect:function(index,row){
                ...
                if ($('#AmsArchiveMemberTabs').length>0){
                    if ($('#AmsArchiveMemberTabs').tabs("tabs").length>1){
                        $('#AmsArchiveMemberTabs').tabs("showHeader");
                    }
                }
                var height = $('#AmsArchiveMemberTabs').outerHeight();
                if ($('#AmsArchiveMemberTabs').tabs('exists',"原文管理")){
                    var tab = $('#AmsArchiveMemberTabs').tabs('getTab',"原文管理");
                    tab.css("height",height+3);
                    var para = "?mid="+row.daglId;
    ...
    if ($("#AmsArchiveMemberTabs").length>0){
        $("#AmsArchiveMemberTabs").tabs({
            onSelect:function (title,index) {
                $.AmsArchive.updateTab($('#AmsArchiveMemberTabs').tabs('getTab',index));
            },
            onUpdate:function (title,index) {
                var tab = $('#AmsArchiveMemberTabs').tabs('getTab',index);
                if (tab.is(":visible")){
                    $.AmsArchive.updateTab(tab);
                }
            }
        })
    }
