...
;(function ($) {
    ...
    $.fn.cabinet = function (options) {
        ...
        this.showMask = function () {
            ...
            if ($('.c-mask').length === 0 || $('.c-layer').length === 0) {
                ...
                html += '       </div>        ' +
                    ...
                    '                <span class="pull-left">我的选择:</span>' +
                    '                <span class="r"></span>' +
                    '            </div>            ' +
            ...
            $('.cabinet-item').on('click', function (e) {
                ...
                if (data.rackAddress != undefined) {
                    ...
                    if (getQueryString('flag')=='view') {//密集柜视图页面
                        $('.c-line span:first').html('我的选择：'+positions.toString());
                        $('.btn-group1').hide();
                    }
                    if(getQueryString('flag')=='rebox'){//重新上架页面
                        $('.btn-group1').hide();
                    }
        ...
        // 调用插件
        try {
            console.log('_options', _options, this)
            this.empty()
            this.initialize()
            this.select(0)
            this.initTab()
